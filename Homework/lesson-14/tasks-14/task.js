let dataList = [
   ...
   // 1) Загальну вартість (нові ціни - price)
        let totalCost = dataList.reduce((prevSum, priceProduct)=>prevSum+priceProduct.price,0)
        document.write (`1) Загальнa вартість (нові ціни - price) => ${totalCost} </br>`)
// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
    // function getNumberProduct(dataList){
    //     let resNum = 0
    ...
    //     return resNum
    // }
    //     let numberProducts = getNumberProduct(dataList)
        let numberProducts = 0
        for (const product of dataList) {
            if (product.price < product.old_price)
            numberProducts++
        }
        document.write (`2) Kількість товарів, у яких ціна зменшилась (price < old_price) => ${numberProducts}</br>`)

// 3) Товари, які доступні (sell_status:"available")
    document.write (`3) Товари, які доступні (sell_status:"available") =>  console.log </br>`)
        let arrProduct = []//перший варіан 
        for (const product of dataList) {
            if(product.sell_status === "available")
            arrProduct.push( JSON.parse(JSON.stringify(product)))
        }
            console.log (`Товари, які доступні (sell_status:"available") => `)
            console.log (arrProduct)
        let arrProduct2 = dataList.filter((product)=> product.sell_status==="available")//другий варіант
            console.log (arrProduct2)       
...
//нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price)
    document.write (`4) сформувати новий список об”єктів тільки доступних  для продажу товарів, які міститимуть тільки ідентифікатор товару</br> (id), 
    нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price) =>  console.log `)
        let listDataNew = []
        for (const product of dataList) {
            listDataNew.push({id: product.id, price: product.price, old_price: product.old_price, usd_price: product.usd_price})
        }
